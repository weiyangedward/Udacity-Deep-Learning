...
def conv_cnn(train_dataset, train_labels, test_dataset, test_labels, valid_dataset, valid_labels):
    batch_size = 16
    patch_size = 5
    depth = 16
    num_hidden = 64
...
# conv_cnn(train_dataset, train_labels, test_dataset, test_labels, valid_dataset, valid_labels)


def conv_pool_cnn(train_dataset, train_labels, test_dataset, test_labels, valid_dataset, valid_labels):
    """
    Problem 1
    ...
    with graph.as_default():
        ...
        # Model.
        def model(data):
            conv_1 = tf.nn.conv2d(data, filter=layer1_weights, strides=[1, 1, 1, 1], padding='SAME')
            hidden_1 = tf.nn.relu(conv_1 + layer1_biases)
            pool_1 = tf.nn.max_pool(hidden_1, [1, 2, 2, 1], [1, 2, 2, 1], padding='SAME')

            conv_2 = tf.nn.conv2d(pool_1, filter=layer2_weights, strides=[1, 1, 1, 1], padding='SAME')
...
# conv_pool_cnn(train_dataset, train_labels, test_dataset, test_labels, valid_dataset, valid_labels)
...
def inception_cnn(train_dataset, train_labels, test_dataset, test_labels, valid_dataset, valid_labels):
    ...
    batch_size = 128
    patch_size = 5
    conv_depth0, conv_depth1, conv_depth2, conv_depth3, conv_depth4 = 16, 16, 48, 48, 120
    ...
    with graph.as_default():
        ...
        # conv3_weights_five_by_five, conv3_biases, conv3_depth = conv_weights(conv_depth1, conv_depth2)
        # conv4_weights_five_by_five, conv4_biases, conv4_depth = conv_weights(conv_depth2, conv_depth2)
        ...
        # layer3_weights_one_by_one, layer3_weights_one_by_one_prefil, layer3_weights_one_by_one_pool, \
        # layer3_weights_three_by_three, \
        # layer3_weights_five_by_five, \
        # layer3_biases, layer3_depth = inception_weights(layer2_depth, conv_depth4)
        ...
        hidden_layer_biases = hidden_weights(image_size // (max_pool_strides ** conv_layer_count) * image_size // (max_pool_strides ** conv_layer_count) * conv2_depth, num_labels, fully_connected_layer_num)
        ...
        def conv_layer(data, conv_weights_five_by_five, conv_biases):
            ...
            # pool = tf.nn.max_pool(conv_five_by_five_relu, ksize=[1, 2, 2, 1], strides=[1, max_pool_strides, max_pool_strides, 1], padding='SAME')
        ...
        def model_train(data):
            ...
            conv_2 = conv_layer(conv_1_pool, conv2_weights_five_by_five, conv2_biases)
            conv_2_pool = max_pool(conv_2)

            # conv_3 = conv_layer(conv_2_pool, conv3_weights_five_by_five, conv3_biases)
            # conv_4 = conv_layer(conv_3, conv4_weights_five_by_five, conv4_biases)
            # conv_4_pool = max_pool(conv_4)
            ...
            # inception_3 = inception_module(inception_2,  layer3_weights_one_by_one_prefil, layer3_weights_one_by_one_pool, layer3_weights_one_by_one, layer3_weights_three_by_three, layer3_weights_five_by_five, layer3_biases)

            # inception_2_pool = max_pool(inception_2)

            shape = conv_2_pool.get_shape().as_list()
            reshape = tf.reshape(conv_2_pool, [shape[0], shape[1] * shape[2] * shape[3]])
        ...
        def model_pred(data):
            ...
            # inception_3 = inception_module(inception_2, layer3_weights_one_by_one_prefil, layer3_weights_one_by_one_pool,
            #                                layer3_weights_one_by_one, layer3_weights_three_by_three,
            #                                layer3_weights_five_by_five, layer3_biases)
